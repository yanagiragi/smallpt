...
Vec radiance(const Ray &ray, int depth, unsigned short *Xi, int includeEmissive=1) {
	...
	Vec origin;
	Shape &hitShape = (id > numSpheres) ? (Shape&)(Triangles[id - 1 - numSpheres]) : (Shape&)(spheres[id]);

	if(id == 11) {
		origin = Triangles[id - 1 - numSpheres].p1 +
 			Triangles[id - 1 - numSpheres].p2 +
			Triangles[id - 1 - numSpheres].p3;
