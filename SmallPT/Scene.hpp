...
class SceneS
{
	...
	void LoadSphere()
	{
		//if (strncmp(globalConfig::modelName, "null", 4) != 0)
		//	LoadModel(globalConfig::modelName);
	...
	void LoadModel(char *filename)
	{
		...
		{
			Triangle tri = Triangle(obj2.positions[i + 0], obj2.positions[i + 1], obj2.positions[i + 2], Vec(), Vec(.999, .999, .999), REFR);
			//Triangles.push_back(tri);
		}
	}
